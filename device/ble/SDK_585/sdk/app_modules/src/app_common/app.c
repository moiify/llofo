...
timer_hnd adv_timer_id                                                      __attribute__((section("retention_mem_area0"),zero_init)); // @RETENTION MEMORY

void (*adv_timeout_callback)(void)                                          __attribute__((section("retention_mem_area0"),zero_init)); // @RETENTION MEMORY
...
static struct gapm_start_advertise_cmd* app_easy_gap_undirected_advertise_start_create_msg(void)
{
    ...
    if (adv_cmd == NULL)
    {
        ...
        cmd->op.code = GAPM_ADV_UNDIRECT;//非定向广播，GAPM_ADV_NON_CONN
        cmd->op.addr_src = user_adv_conf.addr_src;
			
        cmd->intv_min = user_adv_conf.intv_min;
        cmd->intv_max = user_adv_conf.intv_max;
			
        cmd->channel_map = user_adv_conf.channel_map;//37,38,39三个信道广播
        cmd->info.host.mode = user_adv_conf.mode;//可被通用发现
        cmd->info.host.adv_filt_policy = user_adv_conf.adv_filt_policy;
				
        cmd->info.host.adv_data_len = ble_adv_load_advData(cmd->info.host.adv_data);
        cmd->info.host.scan_rsp_data_len = ble_adv_load_scanRspData(cmd->info.host.scan_rsp_data);
 ...
 ****************************************************************************************
 */
void app_easy_gap_advertise_stop_handler(void)
{
    ...
    if(adv_timeout_callback!=NULL)
    {
        ...
        adv_timeout_callback = NULL;//6.adv_data_update_timer_cb
        timeout_callback();
    }
}
