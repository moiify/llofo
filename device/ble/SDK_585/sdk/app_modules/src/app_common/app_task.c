    ...
    switch(param->operation)
    {
        ...
        case GAPC_UPDATE_PARAMS:
        {
            if ((param->status != CO_ERROR_NO_ERROR))
            {
                ...
                CALLBACK_ARGS_2(user_app_callbacks.app_on_update_params_rejected, param->status, src_id)
            }
            else
            {
                CALLBACK_ARGS_1(user_app_callbacks.app_on_update_params_complete, src_id)
            }
        }
        break;
...
const struct ke_msg_handler app_default_state[] =
{
    ...
    {APP_TIMER_USER_MES,                    (ke_msg_func_t)app_user_timer_handler},
};
