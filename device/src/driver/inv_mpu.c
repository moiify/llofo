...
int mpu_whoami(void)
{
    ...
    {
        LOG_ERROR("i2c_read error");
        return -1;
    }

    LOG_DEBUG("read mpu whoami: %d", data[0]);
    if(data[0] != 0x70)
    {
        return -1;
    }
...
int mpu_init(void)
{
     ...
     */
    data[0] = BIT_FIFO_SIZE_1024 | 0x8 | 0x1;
    ...
    mpu_lp_motion_interrupt(100, 20, 1);
...
//进入运动休眠模式
int mpu_wake_on_motion_interrupt(void)
{
